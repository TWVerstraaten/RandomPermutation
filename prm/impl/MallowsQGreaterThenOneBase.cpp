...
namespace prm::impl {
    ...
    MallowsQGreaterThenOneBase::MallowsQGreaterThenOneBase(const double q, size_t unmatched)
        : MallowsBase(q, unmatched), m_safe_power_limit(std::min(math::floor_of_non_negative(this->log_base_q(std::numeric_limits<double>::max()) - 1), this->m_unmatched)) {
    }

    size_t MallowsQGreaterThenOneBase::get_k(const double uniform_zero_one) const {
        size_t k;
        if (this->m_unmatched <= m_safe_power_limit) {
            k = math::ceil_of_non_negative(this->log_base_q(1.0 - uniform_zero_one * (1.0 - std::pow(this->m_q, this->m_unmatched))));
        } else {
            k = approximate_k(uniform_zero_one);
        }
        return math::clamp(k, 1ul, this->m_unmatched) - 1;
    }
